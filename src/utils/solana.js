...
import { PublicKey, Transaction, SystemProgram, TransactionInstruction, Keypair } from "@solana/web3.js";
 ...
 */
export async function getOrCreateAssociatedTokenAccount(connection, wallet, mint, owner) {
...
export async function executeDeposit(
    ...
    try {
        ...
        const [recipientInfo] = getRecipientInfoPDA(userPubkey, PRESALE_CONFIG.rewardToken);
        ...
        const [vestingAccount, vestingAccountBump] = getVestingAccountPDA(PRESALE_CONFIG.rewardToken);
...
export async function executeWithdraw(
    ...
    try {
        ...
        // Get PDAs
        const { vestingAccount, vestingAccountBump, vestingVault, vestingVaultBump } = getVestingPDAs();
        const [vestingInfo, vestingInfoBump] = getVestingInfoPDA(userPubkey, PRESALE_CONFIG.rewardToken);
...
export async function checkUSDCBalance(connection, userPubkey) {
    try {
        const userUSDCAccount = await getUserUSDCAccount(connection, null, userPubkey);
...
export async function checkCORPBalance(connection, userPubkey) {
    try {
        const userCORPAccount = await getUserCORPAccount(connection, null, userPubkey);
        const accountInfo = await connection.getTokenAccountBalance(userCORPAccount);
        return accountInfo.value.uiAmount;
    } catch (error) {
...
export async function fetchPresaleConfig(connection, wallet = dummyWallet()) {
    try {
        ...
        return {
            ...
            initialRewardsAmount: (presaleData.initialRewardsAmount).toNumber() / 1e9,
            totalParticipants: presaleData.totalParticipants.toNumber(),
            publicSoldAmount: presaleData.publicSoldAmount.toNumber() / 1e9,
...
export async function getVestingInfo(connection, userPubkey, wallet = dummyWallet()) {
    try {
        ...
        return {
            totalAmount: vestingData.totalAmount.toNumber() / 1e9,
            withdrawnAmount: vestingData.withdrawnAmount.toNumber() / 1e9,
        };
    } catch (error) {
 ...
 */
export async function getVestingAccount(connection, userPubkey, wallet = dummyWallet()) {
    try {
        const program = getVestingProgram(connection, wallet);
